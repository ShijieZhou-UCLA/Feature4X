...
@torch.no_grad()
def raft_process_folder(model, src_dir, dst_dir, dst_viz_dir, pair_list):
    img_shape = None
    ...
    for vi, vj in tqdm(pair_list):
        image1 = load_image(osp.join(src_dir, vi)).to(device)
        image2 = load_image(osp.join(src_dir, vj)).to(device)
        if img_shape is None:
            # load image already handle the resizing
            img_shape = imageio.imread(osp.join(src_dir, vi)).shape[:2]
        ...
        # Save flow
        np.savez(
            os.path.join(dst_dir, f"{vi}_to_{vj}.npz"), flow=flow_fwd, mask=mask_fwd
        )
        np.savez(
            os.path.join(dst_dir, f"{vj}_to_{vi}.npz"), flow=flow_bwd, mask=mask_bwd
        )

        # Save flow_img
        flow_viz_fwd = flow_viz.flow_to_image(flow_fwd)
        cv2.imwrite(
            os.path.join(dst_viz_dir, f"{vi}_to_{vj}.jpg"),
            flow_viz.flow_to_image(flow_fwd),
        )
        flow_viz_list.append(flow_viz_fwd)
        cv2.imwrite(
            os.path.join(dst_viz_dir, f"{vj}_to_{vi}.jpg"),
            flow_viz.flow_to_image(flow_bwd),
        )
        cv2.imwrite(
            os.path.join(dst_viz_dir, f"{vi}_to_{vj}_mask.jpg"),
            mask_fwd.astype(np.uint8) * 255,
        )
        cv2.imwrite(
            os.path.join(dst_viz_dir, f"{vj}_to_{vi}_mask.jpg"),
            mask_bwd.astype(np.uint8) * 255,
        )
    imageio.mimsave(
